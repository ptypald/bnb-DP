...
static void read_P(void) {
	...
	while (fgets(buf, sizeof(buf), stdin))
	{
		...
		if (sscanf_s(buf, "\"numsteps\":%d", &ival) == 1)
			P.numsteps = ival;
		if (sscanf_s(buf, "\"safety\":%lf\n", &dval) == 1)
			P.safety = 1.0 - dval;
		if (sscanf_s(buf, "\"C\":%lf", &dval) == 1)
			P.C = dval;
		if (sscanf_s(buf, "\"T\":%lf", &dval) == 1)
			P.T = dval;
		if (sscanf_s(buf, "\"x(0)\":%lf", &dval) == 1)
			P.x0 = dval;
		if (sscanf_s(buf, "\"y(0)\":%lf", &dval) == 1)
			P.y0 = dval;
		if (sscanf_s(buf, "\"v(0)\":%lf", &dval) == 1)
			P.v0 = dval;
		if (sscanf_s(buf, "\"a(0)\":%lf\n", &dval) == 1)
			P.a0 = dval;
		if (sscanf_s(buf, "\"obst_x(%d,0)\":%lf", &ival, &dval) == 2) {
			P.obst_x[ival] = dval;
            P.obst_x_len = MAX(P.obst_x_len, ival + 1);
		}
		if (sscanf_s(buf, "\"obst_y(%d,0)\":%lf", &ival, &dval) == 2) {
			P.obst_y[ival] = dval;
		}
		if (sscanf_s(buf, "\"obst_v(%d,0)\":%lf", &ival, &dval) == 2) {
			P.obst_v[ival] = dval;
		}		
...
static void init_S(int numsteps, double T, int n, double *obst_x, double *obst_y, double *obst_v) {
	int i, k;
...
static void init_D(double T) {
	...
	/* v domain: dv is chosen arbitrarily small */
					...
					*Jhere = DBL_MAX;
					...
					//for (u = -3.3; u <= 3.3; u += 3.3) {
						...
						for (dy = -1; dy <= 1; dy++) {
							...
							phi = pow((P.vd - here.v), 2) + 15.0*pow(u, 2);
...
int main(int argc, char **argv) {
	...
	fprintf(stderr, "CPU Time: %.4f", cpu_time_used);
